...
\todo{Insert exploded view diagram or CAD render of the redesigned device.}

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.7\textwidth]{figures/exploded_view_dfd.pdf}
%     \caption{Exploded view of the DfD-redesigned e-cigarette with universal removable battery.}
%     \label{fig:exploded_dfd}
% \end{figure}

Key design changes to the device body:
\begin{enumerate}
    \item \textbf{Housing material}: Switch from aluminum to \textbf{mono-material recyclable plastic} (polypropylene or recycled PET) to eliminate mixed-material contamination and reduce embodied energy
    \item \textbf{Snap-fit assembly}: Two-piece clamshell housing with integrated snap-fit clips --- no adhesives, no ultrasonic welding, no screws
    \item \textbf{Battery compartment}: Molded receptacle at the base with spring contacts and twist-lock door
    ...
    \item \textbf{Standardized PCB mounting}: PCB secured with press-fit standoffs, removable without tools
\end{enumerate}
