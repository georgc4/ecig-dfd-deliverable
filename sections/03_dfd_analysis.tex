...
\textbf{Current design}: The housing is sealed using adhesive bonding, with certain components, such as the PCB, further being secured via a press-fit. Disassembly required extensive cutting with a thin X-acto knife, damaging both the housing and internal components. This process was particularly cumbersome, requiring sustained lateral force along the seam to separate the aluminum outer shell from the inner plastic sleeve.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\linewidth]{snap.png}
    \caption{Reversible snap-fit connectors}
    \label{fig:snapfit_diagram}
\end{figure}

\subsection{Principle 2: Standardize and Minimize Fasteners}

\textbf{Current design}: The Air Bar NEX uses no screws. All internal components are held in place via press-fit tolerances and adhesive bonding. The PCB is wedged against the battery and reservoir with no independent retention, and the mouthpiece cap is friction-fit onto the top of the housing.
...
\textbf{Current design}: The Air Bar NEX carries no material identification markings on any component. The outer aluminum housing, inner plastic sleeve, and mouthpiece bear no recycling symbols, ISO resin codes, or battery chemistry labels. The only external markings are brand graphics and regulatory compliance text (CE, UKCA).
...
\textbf{Current design}: Full disassembly of the Air Bar NEX required 8 distinct operations: scoring the housing seam, prying off the aluminum shell, extracting the inner sleeve, cutting the reservoir seal, desoldering PCB leads, separating the battery from its adhesive pad, removing the coil assembly, and pulling the mouthpiece cap. The process took approximately 10--15 minutes and required an X-acto knife, pliers, and a soldering iron.
...
\midrule
Steps to remove battery       & 8               & 1 \\
Total disassembly steps        & 8               & $\leq$5 \\
Tools required                 & pliers, knife, soldering iron & None (hand only) \\
Disassembly time               & 10--15 min    & $<$30 sec \\
Destructive?                   & Yes             & No \\
...
\begin{figure}[H]
    ...
    \caption{Disassembling the current design takes 10+ minutes and requires handheld tools.}
    \label{fig:disassembly_current}
\end{figure}